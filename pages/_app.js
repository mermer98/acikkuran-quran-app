...
const Acikkuran = (props) => {
  ...
  // Hata almamak için, locale bulunamazsa fallback yap
  let manifest = "/manifest.json";
  const localeConfigObj = localesConfig.find((i) => i.locale === locale);
  if (localeConfigObj && localeConfigObj.manifest) {
    manifest = localeConfigObj.manifest;
  }
...
Acikkuran.getInitialProps = async (appContext) => {
  ...
  const currentDomain = req?.headers?.host || "acikkuran.com";

  // Eğer locale gelmezse, varsayılanı "tr" yap
  const locale =
    appContext.ctx.locale ||
    process.env.NEXT_PUBLIC_LOCALE ||
    "tr";
  const defaultAuthorId = defaultAuthors[locale] || 1;
