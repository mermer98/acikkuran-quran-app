...
def fix_arabic(text: str) -> str:
    if text is None:
        return ""
    # reshape for joining, then compute display order
...
class KuranGUI(ctk.CTk):
    def __init__(self):
        ...
        # Sonuç kutusu
        self.result_box = Text(right_panel, font=("Traditional Arabic", 16), wrap="word")
    ...
    def update_results(self, df, keyword=None):
        ...
        for idx, row in df.iterrows():
            metin = fix_arabic(row['metin'])
            ayet_line = f"{metin}  —  {row['sure']}/{row['ayet']}  [{row['sira']}]"
        ...
        self.result_box.tag_configure("rtl", justify="right")
        self.result_box.tag_add("rtl", "1.0", "end")
