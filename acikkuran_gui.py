...
DEFAULT_MEAL = "Mehmet_Okuyan_arapca.csv"     # varsayılan dosya adı (klasörde olmalı)
...
class KuranGUI(ctk.CTk):
    def load_arapca_ayetler(self):
        try:
            self.arapca_ayetler_df = pd.read_csv('arapca_ayetler.csv', dtype=str)
        except Exception:
            pass
    def __init__(self):
        super().__init__()
        ...
        self.arapca_ayetler_df = None
        self.load_arapca_ayetler()
    ...
    def update_results(self, df, keyword=None):
        ...
        for idx, row in df.iterrows():
            metin = row['metin']
            arapca_line = ""
            # Arapça metni arapca_ayetler.csv'den al
            arapca = None
            if self.arapca_ayetler_df is not None:
                match = self.arapca_ayetler_df[(self.arapca_ayetler_df['sure'] == str(row['sure'])) & (self.arapca_ayetler_df['ayet'] == str(row['ayet']))]
                if not match.empty:
                    arapca = match.iloc[0]['metin']
            if arapca:
                arapca_lines = []
                for line in arapca.split('\n'):
                    reshaped = arabic_reshaper.reshape(line)
                    bidi_text = get_display(reshaped)
                    arapca_lines.append(bidi_text)
                arapca_line = '\n' + '\n'.join(arapca_lines)
            ayet_line = f"[{row['sira']}] {row['sure']}/{row['ayet']} — {metin}{arapca_line}"
